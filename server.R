...
shinyServer(function(input, output, session) {
  ...
  pushCheck <- function(key){
    ...
    temp[, reportStart:=input$reportStart]
    temp[, reportEnd:=input$reportEnd]
    print("Stop point 1")
  ...
  observeEvent(input$addToRemote, {
    ...
    if(password2==password){
      #pushToMaster(source_key)
      pushCheck(source_key)
      outTable <- outTable[SourceURL!=input$sourceURL]
      print("bottom one")
      showNotification("Succesfully added!", duration=3, type="message")
    }
  })
  ...
  ###Below: function for creating individual session sheets
  ...
  ## Below: function for adding to temporary R session master sheet
...
###########
  ...
  #Below: deprecated ESTMA button function where tables were auto-detected; replaced in favor of manual page-by-page detection (above)
  ...
  ################
  #Pick table to work on (deprecated along with ESTMA load *&* scrape)
  ...
  slLink <- "https://docs.google.com/spreadsheets/d/1cbumxhU4vhWxp96LI5plcN_t5jQusVeVPhNZ7qU4wDM/pub?output=csv"
  
  #outTable <- gs_read(slist_key, range=cell_cols(1:9))
  ...
  print(names(outTable))
  outTable[, c("Company","Link","SourceType","Format","filed_in")]
  ...
  observeEvent(input$loadSourcelist, {
    ...
    output$sourcelistPDF <- renderTable({
      outTable[Scraped==0 & SourceType=="ESTMA" & ReportType..ESTMA.only.=="Individual", c("Company","Link","SourceType","Format","filed_in")]
      }, sanitize.text.function = function(x) x)
    
    output$sourcelistExcel <- renderTable({
      outTable[Scraped==0 & SourceType=="ESTMA" & ReportType..ESTMA.only.=="Consolidated", c("Company","Link","SourceType","Format","filed_in")]
    }, sanitize.text.function = function(x) x)
    
    
    output$sourcelistTBD <- renderTable({
     outTable[Scraped==0 & SourceType=="Other", c("Company","Link","SourceType","Format","filed_in")]
     }, sanitize.text.function = function(x) x)
